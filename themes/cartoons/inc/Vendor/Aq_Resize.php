...
function aq_add_image_size( $name, $width = 0, $height = 0, $crop = false, $upscale = true ){
    global $_aq_image_sizes;
    
    $_aq_image_sizes[ $name ] = array(
        'width'     => absint( $width ),
...
function aq_get_attachment_image_src( $image, $attachment_id, $size, $icon ) {
    $system_sizes = get_intermediate_image_sizes();
    $system_sizes[] = 'full';
    if( !empty($image) && (!is_admin() || wp_doing_ajax()) && !in_array($size, $system_sizes) ) {
        $_aq_image_sizes = aq_get_image_sizes();
        if( !is_array($size) && !empty($_aq_image_sizes[$size]) ) {
            extract($_aq_image_sizes[$size]);
        } else if( !empty($size) && is_array($size) && count($size)==2 ) {
            $width = $size[0];
            $height = $size[1];
            $crop = true;
            $upscale = true;
        }
        $img_src = preg_replace('/-[\d]+x[\d]+(.jpg|jpeg|png|gif)$/', "$1", $image[0]);
        $resized = aq_resize( $img_src, $width, $height, $crop, false, $upscale ); 
        return $resized;
    }
    return $image;
}
add_filter( 'wp_get_attachment_image_src', 'aq_get_attachment_image_src', 10, 4 );

function aq_get_image_sizes() {
    global $_aq_image_sizes;
    if ( ! $_aq_image_sizes ) {
        $_aq_image_sizes = array();
    }
    return $_aq_image_sizes;
}